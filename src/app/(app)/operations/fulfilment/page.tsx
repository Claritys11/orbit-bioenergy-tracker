...
export default async function FulfilmentPage() {
  await requireUser("fulfil_allocation");
  ...
  return (
    <div className="grid gap-6">
      <PageHeader title="Allocation Fulfilment" description="Track pending, scheduled, partially fulfilled, delivered, cancelled, and rolled-over energy allocations without overstating delivery." />
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card>
          ...
          <form action={createFulfilmentFormAction} className="mt-4 grid gap-4">
            <SelectField label="Allocation" name="allocationId" required options={allocations.map((allocation) => ({ value: allocation.id, label: `${allocation.cycle.cycleCode} - ${allocation.pool} - ${formatGas(allocation.allocatedGasM3)}` }))} />
            <SelectField label="Status" name="status" required options={[
              "PENDING", "SCHEDULED", "DELIVERED_PHYSICALLY", "CONSUMED_AT_HUB", "PARTIALLY_FULFILLED", "CANCELLED", "ROLLED_OVER",
            ...
            <SelectField label="Delivery mode" name="deliveryMode" required options={[
